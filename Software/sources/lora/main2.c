...
void hexdump(const uint8_t *buf, size_t len) {
	...
	if (len && last_mod - 7) {
		cbuf[last_mod + 1] = '\n';
		cbuf[last_mod + 2] = 0;
		for (size_t i = last_mod + 1; i < 8; i += 1) {
			printf("   ");
		}
...
void Lora_Process_RX() {
	...
	printf("-- RX RF: RSSI: %d, SNR: %d, RSCP: %d\n",
...
void Lora_SendPkt(void *buf, size_t len) {
	sx126x_pkt_params_lora_t lora_pkt_params = {
		.preamble_len_in_symb = 16,
		.header_type = SX126X_LORA_PKT_EXPLICIT,
	...
	printf("-- Sending packet: len=%zu\n", len);

	sx126x_set_lora_pkt_params(NULL, &lora_pkt_params);
	sx126x_write_buffer(NULL, 0, buf, len);
	sx126x_set_tx(NULL, SX126X_MAX_TIMEOUT_IN_MS);
